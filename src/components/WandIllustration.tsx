...
// Different wand-pointing motions for each spell
const wandMotions = [
  // Illuminate: quick flick upward then point
  { rotate: [0, -6, 2, 0], y: [0, -4, 1, 0], transition: { duration: 0.6, times: [0, 0.3, 0.7, 1] } },
  // TV-onus: smooth swoop from left
...
export default function WandIllustration({ isCasting = true, spellIndex = 0 }: { isCasting?: boolean; spellIndex?: number }) {
  const motionVariant = wandMotions[spellIndex % wandMotions.length];
  ...
  return (
    ...
    >
      {/* Wand-pointing animation wrapper */}
      <motion.div
        animate={isCasting ? {
      ...
      >
        {/* Ambient glow behind the boy and wand — use CSS instead of extra DOM */}
        ...
        >
          ...
          <AnimatePresence>
            {isCasting && (
              ...
              >
                {/* Core beam — no filter, just opacity for glow effect */}
                <path d="M350 180 Q420 178 500 180" stroke="url(#beamGradient)" strokeWidth="8" strokeLinecap="round" opacity="0.5" className="animate-pulse" />
                <path d="M350 180 Q420 178 500 180" stroke="url(#beamGradient)" strokeWidth="4" strokeLinecap="round" />
                <path d="M350 180 Q420 175 500 170" stroke="url(#beamGradient)" strokeWidth="3" strokeLinecap="round" opacity="0.6" />
                <path d="M350 180 Q420 185 500 190" stroke="url(#beamGradient)" strokeWidth="3" strokeLinecap="round" opacity="0.6" />

                {/* Fewer particles along the beam — reduced from 15 to 6 */}
                {[
                ...
                {/* Sparkle stars at tip — no filter */}
                ...
                {/* === ORB GLOW at wand tip — no SVG filter === */}
                {/* Outer glow ring */}
                <circle cx="355" cy="180" r="45" fill="url(#orbGlowOuter)">
                  <animate attributeName="r" values="40;50;40" dur="2s" repeatCount="indefinite" />
                  <animate attributeName="opacity" values="0.6;0.9;0.6" dur="2s" repeatCount="indefinite" />
                </circle>
                {/* Inner orb */}
                <circle cx="355" cy="180" r="18" fill="url(#orbGlow)">
          ...
          {/* Small gem at base — no filter */}
          <ellipse cx="108" cy="200" rx="4" ry="6" fill="#7b4bb3" opacity="0.8" />
